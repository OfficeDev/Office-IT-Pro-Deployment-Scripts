...
#This example will detect the current install of Office that is currently installed. If the current install of Office Click-To-Run 64-bit it will
...
#and change the configuration XML to 32-Bit.  It will remove the existing install of Office Click-To-Run and resinstall Office Click-To-Run with the 32-Bit version

$installOffice = $true

$officeC2R = getCTRConfig
if ($officeC2R) {
    if ($officeC2R.Platform -eq "64") {
...
if ($installOffice) {
  if (Test-Path -Path $targetFilePath) {
      ...
      Set-ODTAdd -TargetFilePath $targetFilePath -Version $NULL -Bitness 32 -SourcePath $SourcePath | Out-Null
