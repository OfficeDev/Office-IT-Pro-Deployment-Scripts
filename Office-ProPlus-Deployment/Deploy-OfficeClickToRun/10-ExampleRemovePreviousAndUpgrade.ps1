...
if ($Office2016C2RExists) {
  Write-Host "Office 2016 Click-To-Run is already installed"
    ...
    if (!(Test-Path -Path $targetFilePath)) {
       Generate-ODTConfigurationXml -Languages AllInUseLanguages -TargetFilePath $targetFilePath | Set-ODTAdd -Version $NULL -SourcePath $SourcePath -Channel SemiAnnual | Set-ODTDisplay -Level None -AcceptEULA $true | Out-Null
